\chapter{Die Ungenauigkeit von Schätzungen einschätzen}\label{ch:uncertainty}
Eine unumgängliche Gegebenheit beim Arbeiten mit Stichproben
ist, dass wir Eigenschaften von Populationen
nur \emph{schätzen} können. Die Frage stellt sich, wie
...
Dazu introduziert dieses Kapitel den sog.\ \term{Bootstrap},
...
(vgl.\ Abschnitt \ref{sec:clt}) das Gleiche machen kann.
...
und die fünf Verteilungen der gebootstrappten Mittel. Die Perzentile und der Unterschied zwischen ihnen stimmen wegen Rundung nicht komplett miteinander übrein.}
\label{tab:bootstrap}
\begin{tabular}{lrrrr}
\toprule
...
\begin{itemize}
 ...
 \item Der Bootstrap ist flexibel. Hier haben wir uns mit der Unsicherheit eines
 ...
 Perzentils, oder irgendwelcher anderen Masse. Dies wird in den Aufgaben gemacht.
 ...
 Dieser Punkt wird später in diesem Kapitel deutlicher werden,
 aber hier sei bereits darauf hingewiesen, dass wir in den
...
\begin{itemize}
 ...
 Stichproben im Allgemeinen: Andere Verfahren bieten hier keine bessere Lösung.\footnote{Ausser 
 ...
 Daher muss man den Bootstrap meistens selber programmieren. \parend
\end{itemize}
...
$\lfloor \alpha n \rfloor$. Wir entfernen dann die $\lfloor \alpha n\rfloor$
...
Falls mehrere GJT-Beobachtungen den gleichen Rang haben, werden ihre Ränge
...
kleiner als 72 (bzw.\ kleiner-gleich 71) berechnet:
<<>>=
n <- length(d$GJT)
alpha <- 0.2
...
anhand des Bootstraps. Dazu brauchen Sie lediglich eine Zeile aus dem
Code unter Beispiel \ref{bsp:bootstrap} zu ändern. Zeichnen Sie auch
...
\mypar[Gosset (Student)]{Satz}\label{th:gosset}
  ...
  Man schreibt $T \sim t_{n-1}$ oder $T \sim t(1)$.
\parend
...
`Student' war übrigens das Pseudonym von William S.\ Gosset.
...
das 2.5.\ und das 97.5.\ Perzentil der $t$-Verteilung mit $n-1$ Freiheitsgraden
...
\mypar[Vertrauensschranken und Konfidenzintervall]{Definition}
  ...
  Eine Abbildung $b(X_1, \dots, X_n)$ liefert eine
  \term{untere $100(1-\alpha)$\%-Vertrauensschranke}, wenn
  \[
  ...
  wenn 
  \[
    \Prob(\tilde{a}(\alpha, X_1, \dots, X_n) \leq \theta \leq \tilde{b}(X_1, \dots, X_n))
    \geq 1 - \alpha
  \]
  für alle $\alpha \in (0, 1)$.
\parend
...
konstruieren, dass zu einer Wahrscheinlichkeit von höchstens $\alpha$ den
Parameter $\theta$ nicht enthält.
...
mean(simulation <= -34)
@
Für 87\% der simulierten Stichproben liegen die untere 87\%-Vertrauensschranke also
...
\]
denn im Ausdruck `$16.58 \leq \mu$' kommt keine Zufälligkeit vor.\footnote{Ein anderes Beispiel:
...
\mypar[Gütekriterien für Konfidenzintervalle]{Bemerkung}
  ...
  Es ist möglich, sich Verfahren einfallen zu lassen, die zwar die nötigen
...
\mypar[Exakte Vertrauensschranken für das Mittel einer Normalverteilung mit unbekannter Varianz]{Beispiel}\label{bsp:ci_exact_t}
  ...
  schnell berechnen. Den restlichen Output dieser Funktion behandeln
  wir erst später:
<<>>=
...
\mypar[Approximative Vertrauensschranken fürs Mittel]{Beispiel}
...
\mypar[Robustheit der $t$-Methode]{Aufgabe}
...
<<>>=
one_run_uniform <- function(n, min, max, conf_level) {
  x <- runif(n, min, max)
...
<<>>=
simulation <- replicate(20000, one_run_uniform(9, -12, 2, 0.7))
mean(simulation)
@
  In diesem Fall enthalten also etwa 70.5\% der 70\%-Konfidenzintervalle 
  ...
  von \texttt{conf\_level}. Was schlussfolgern Sie?
    ...
    um ihr Mittel. $\chi^2$-Verteilungen sind dahingegen rechtsschief:
...
\mypar[Exakte Vertrauensschranken für Binomialparameter]{Beispiel}
  ...
  \[
    \widehat{p} := \frac{X}{n}.
  \]
  Wir wollen nun untere und obere exakte Vertrauensschranken für $p$ konstruieren.
  ...
  \end{align*}
  Wir fassen den Ausdruck $F_{n,p}(r)$ diesmal jedoch nicht auf als eine Funktion von $r$,
  sondern als eine Funktion von $p$. Für $r$ setzen wir die beobachtete
  Anzahl Erfolge $X$ ein. Die Funktion, die wir so erhalten, ist monoton fallend
  ...
  \begin{align*}
    \Prob(p \leq b)
    &= \Prob(F_{n,p}(X) \geq F_{n,b}(X)) & [\textrm{monoton fallend}] \\
...
curve(pbinom(X, n, x), from = 0, to = 1,
      xlab = "p", ylab = "F(23,p)({7, 23-7})", col = "red")
abline(h = 0.10, lty = 2)
abline(v = 0.46, lty = 2)

# Untere Schranke
curve(pbinom(16, n, x), add = TRUE, col = "blue")
abline(h = 0.10, lty = 2)
abline(v = 1-0.1781578, lty = 2)
@
