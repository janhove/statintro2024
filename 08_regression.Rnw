...
Ist dies nicht möglich und reicht eine grafische Darstellung nicht, 
dann dürften komplexere Verfahren geeignet sein. 
Siehe hierzu \url{https://m-clark.github.io/generalized-additive-models/}, \citet{Wieling2018} und \citet{Baayen2020}.
\parend
...
\mypar[keine Kovarianz, aber nicht unabhängig]{Beispiel}
  ...
  Um dies zu sehen, betrachten wir den Grundraum
  \[
    \Omega := \{(0, 0), (1, -1), (1, 1)\}
  \]
...
Sie es in der Forschungsliteratur nur selten antreffen
...
plot_r(n = 30, r = 0.4)
@
Die Funktionsdokumentation können Sie wie gehabt mit \texttt{?plot\_r} abrufen.
\parend
...
Wenn gewisse $x$- oder $y$-Werte einander gleich sind, 
so wird die Berechnung angepasst, um mit diesen Bindungen umzugehen.
...
<<eval = FALSE>>=
# Histogramm mit den Bootstrap-Schätzungen (nicht gezeigt)
...
und eine Steigung ($\beta_1$; diese sagt, wie viele Punkte $y$ höher liegt, 
wenn $x$ eine Einheit mehr nach rechts liegt);
...
die die Summe der quadrierten Restfehler ($\sum_{i = 1}^{n} \widehat{\varepsilon}_i^2$) minimiert.
Wir können diese Summe für unterschiedliche Kombinationen
von $\widehat{\beta}_0$- und $\widehat{\beta}_1$-Werten berechnen.
\begin{itemize}
\item Sind  $\widehat{\beta}_0 = 185$ und  $\widehat{\beta}_1 = -2$,
dann ist
\[\sum_{i = 1}^{76} \widehat{\varepsilon}_i^2 = \sum_{i = 1}^{76} (y_i - (185 - 2x_i))^2 = 107121.
\]
...
\item Sind  $\widehat{\beta}_0 = 190$ und  $\widehat{\beta}_1 = -1.5$,
dann ist
\[
\sum_{i = 1}^{76} \widehat{\varepsilon}_i^2 = \sum_{i = 1}^{76} (y_i - (190 - 1.5x_i))^2 = 28810.
\]
In R:
<<>>=
...
für unterschiedliche  $(\widehat{\beta}_0, \widehat{\beta}_1)$-Kombinationen;
...
von $x$ bzw.\ $y$. $\overline{x}$ ist das Stichprobenmittel von $x$.
...
\texttt{(Intercept)} ist hier $\widehat{\beta}_0$, also
der Schnittpunkt der Regressionsgeraden mit der $y$-Achse.
\texttt{AOA} ist $\widehat{\beta}_1$ und zeigt, um wie viele
Einheiten die Gerade steigt oder senkt, wenn man entlang
der \texttt{AOA}-Achse eine Einheit nach rechts geht.

Die Maximum-Likelihood-Methode (Abschnitt \ref{sec:ml}) würde 
dieselben $\beta$-Schätzungen liefern.
...
\mypar[andere Optimalitätskriterien]{Bemerkung}
  ...
  (\term{Median-Regression}). Es gibt auch Methoden, um die $\beta$-Parameter
  zu schätzen, beispielsweise solche, in denen gleichzeitig
...
\mypar{Aufgabe}\label{ex:vanhove2014}
...
\[
y_i = \widehat{\beta}_0 + \widehat{\beta}_1x_i + \widehat{\varepsilon}_i, ~~~~~ i = 1, \dots, n.
\]

Auf der Regressionsgeraden ($\widehat{\beta}_0 + \widehat{\beta}_1x_i$)
liegen die $y_i$-Werte abzüglich des Restfehlers, also
\[
\widehat{y}_i = \widehat{\beta}_0 + \widehat{\beta}_1x_i, ~~~~~ i = 1, \dots, n.
\]
Wie man diese $\widehat{y}_i$-Werte konzeptuell interpretieren
...
\begin{enumerate}\label{nonparametricbsregression}
 \item Man berechnet $\widehat{\beta}$ (also $\widehat{\beta}_0$ und $\widehat{\beta}_1$) und erhält dazu auch noch $\widehat{\varepsilon}$.
 \item Man zieht eine Bootstrap-Stichprobe aus $\widehat{\varepsilon}$.
 Nenne diese $\widehat{\varepsilon}^{*}$.
 \item Man kombiniert $\widehat{\beta}_0$, $\widehat{\beta}_1x_i$  und $\widehat{\varepsilon}^{*}$. Dies ergibt
 eine neue Reihe von $y$-Werten: 
 \[
 y_i^{*} = \widehat{\beta}_0 + \widehat{\beta}_1x_i + \widehat{\varepsilon}_i^{*}, i = 1, \dots, n.
 \]
...
apply(bs_beta, 2, sd)
@
Wir haben somit $\widehat{\beta}_0 = 190 \pm 4, \widehat{\beta}_1 = -1.2 \pm 0.1$.
...
Konfidenzbänder mit dem Bootstrap konstruieren kann, aber diese
...
($\widehat{\beta}_0^{*}$) und der Steigung ($\widehat{\beta}_1^{*}$)
...
@
Wegen der Zufälligkeit, die dem Bootstrap inhärent ist,
...
## Alternative mit Matrizenalgebra
# bs_y_hat <- tcrossprod(bs_beta, cbind(1, neue_aoa))
@
...
den genauen Wert der Regressionsgeraden zu verwenden.
<<>>=
...
@
 ...
 Produktionen teilen durch die Wurzel von $25 \cdot 50 = 1250$.

 Typische Verletzungen der Unabhängigkeitsannahme können
 mit sog.\ \term{gemischten Modellen} behoben werden; 
 siehe Kapitel \ref{ch:lmer}.
 Für Verletzungen der Homoskedastizitätsannahme bietet sich
 eine andere Schätzungsmethode als \textit{ordinary least squares} an,
...
\mypar[punktweise vs.\ simultane Konfidenzbänder]{Bemerkung}
  ...
  zu einer Wahrscheinlichkeit von mindestens $1-\alpha$ \emph{alle}
  ...
  so dürfen Sie davon ausgehen, dass es sich um die punktweise Variante handelt.
\parend
...
Der neue Teil $\varepsilon_i \sim \mathcal{N}(0, \sigma_{\varepsilon}^2)$ macht klar,
...
@
  ...
  \[
  \widehat{y}_i = \widehat{\beta}_0 + \widehat{\beta}_1x_i = \overline{y} - \widehat{\beta}_1\overline{x} +  \widehat{\beta}_1x_i.
  \]
  Für $x_i = \overline{x}$ erhalten wir also $\widehat{y}_i = \overline{y}$.
\parend
...
\mypar[Schnittpunkt interpretierbarer machen]{Bemerkung}\label{bm:zentrieren}
...
einem jetzt sofort auch sagt, was das Stichprobenmittel der $y$-Variablen ist.
...
von 35 für das Modell eine Versuchsperson mit einem \texttt{c.AOA}
von $35 - \overline{x}_{AOA} = 2.46$ ist:
<<>>=
predict(aoa.lm, newdata = tibble(c.AOA = 35 - mean(d$AOA)),
        interval = "confidence", level = 0.80)
@

Man kann den Schnittpunkt manchmal auch interpretierbarer machen,
indem man die Prädiktorwerte um einen anderen Wert als ihr Mittel zentriert.
Beispielsweise kann es sich hier um den Median handeln, auch aber um irgendeinen
anderen Wert, der im Kontext der Studie sinnvoll ist.
\parend
