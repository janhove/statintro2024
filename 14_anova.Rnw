...
3 $t$-Tests) 20'000 durchlaufen. Jedes Mal wird der $p$-Wert
...
\mypar{Aufgabe}
  \begin{enumerate}
    \item Es seien $P_1, P_2, P_n$ unabhängige Zufallsvariablen mit
    ...
    Hinweis: Siehe Abbildung \ref{fig:familywise}.
    ...
    haben. Wieso unterscheiden sich diese Zahlen dann schon?
  \end{enumerate}
\parend
...
Wenn Sie sich Aufgabe \ref{ex:ex:randtest_multiple} bereits zur Brust genommen haben,
...
<<>>=
(sum(var_mittel_H0 >= 1.282355) + 1) / 20000
@
...
\item Die Nullhypothese ist fast ausnahmslos, dass das Muster nur aufgrund von
 ...
 stimmen würde. Ist diese Wahrscheinlichkeit niedrig, d.h., unter einer im
