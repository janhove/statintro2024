...
eines `signifikanten' $p$-Wertes zu sein.
...
Die Versuchspersonen in der Experimentalgruppe sollen ein Videofragment
...
die Herkunft der Versuchspersonen in den beiden Gruppen fixiert hätten 
(z.B.\ eine Bünderin, ein Zürcher und eine Bernerin in jeder Gruppe)
...
uns jedoch mit einer trivialeren Erklärung beschäftigen: Vielleicht
...
\begin{itemize}
  ...
  \item $H_A$: Der Unterschied ist \emph{nicht nur} dem Zufallsfaktor
  zuzuschreiben.
\end{itemize}
...
unter welcher der $p$-Wert als zu klein gilt.
...
mit dem binomischen Koeffizienten (vgl.\ Abschnitt \ref{sec:binomial}) 
berechnen:\footnote{Für grössere
...
um eine Gruppe von 40 Teilnehmenden in zwei gleich grosse
Gruppen aufzuteilen.}
\[
  {6\choose 3} = 20.
\]
...
<<echo = FALSE, fig.width = 10, fig.height = 13, out.width=".8\\textwidth", fig.cap = "Die 20 möglichen Ergebnisse laut der Annahme, dass der Unterschied zwischen den Konditionen nur der Randomisierung zuzuschreiben ist.\\label{fig:20moeglichkeiten}">>=
...
\mypar[$p$-Wert]{Definition}\label{def:pvalue}
  Ein (exakter) $p$-Wert ist eine Zufallsvariable $P$,
...
\mypar[weitere Gütekriterien]{Bemerkung}
  ...
  $p$-Wert, da sie immer die gleiche Verteilung hat -- egal, ob die Nullhypothese
  stimmt oder nicht. Auch eine Zufallsvariable $P \sim \textrm{Unif}((0,1))$ stellt
  ein gültiger $p$-Wert dar, denn
  \[
    \Prob(P \leq \alpha) = \alpha \leq \alpha,
  \]
  für alle $\alpha \in (0,1)$.
  Dieser $p$-Wert hat auch immer die gleiche Verteilung, egal ob die Nullhypothese
  stimmt oder nicht.
  Was wir uns eigentlich wünschen, sind also $p$-Werte, die nur selten `klein' sind,
  wenn die Nullhypothese stimmt, aber viel öfters klein sind, wenn 
  sie nicht stimmt.
\parend

Das folgende Lemma erklärt, wieso der $p$-Wert, den wir im Beispiel oben berechnet haben,
überhaupt ein gültiger $p$-Wert im Sinne von Definition \ref{def:pvalue} ist.
Ein Beweis des Lemmas findet sich in \citet[][S.\ 5]{Duembgen2016}.

\mypar{Lemma}\label{lemma:pvalue}
  Sei $X$ eine Zufallsvariable mit Verteilungsfunktion $F_0$. 
  Dann gelten
  \begin{align*}
    \Prob(F_0(X) \leq \alpha) &\leq \alpha,                             \tag{$p_{\ell}$}\\
    \Prob(1 - F_0(X-) \leq \alpha) &\leq \alpha,                        \tag{$p_{r}$}\\
    \Prob(2 \cdot \min\{F_0(X), 1 - F_0(X-)\} \leq \alpha) &\leq \alpha,\tag{$p_z$}
  \end{align*}
  für alle $\alpha \in (0,1)$.
  (Zur Bedeutung von $F_0(X -)$, siehe Abschnitt \ref{sec:verteilungsfunktion}.)
\parend

Dieses Lemma impliziert nämlich, dass wir uns beim Berechnen von $p$-Werten
der folgenden Schablone bedienen dürfen. 
\begin{itemize}
\item Zunächst überlegen wir uns,
für welche Grösse $X$ wir uns überhaupt interessieren.
Im obigen Beispiel interessierten wir uns für
den Unterschied zwischen den Stichprobenmitteln, aber wir werden uns bald
mit anderen Grössen beschäftigten.

\item Wir überlegen uns nun, wie denn diese Grösse $X$ verteilt wäre, 
wenn die Nullhypothese stimmen würde. 
So erhalten wir die Verteilungsfunktion $F_0$ von $X$. 
Im obigen Beispiel ermittelten wir $F_0$, 
indem wir alle möglichen Zuordnungen durchackerten. 
Der Wert $F_0(r)$ entspräche dann der Proportion der Zuordnungen, 
die zu einem Gruppenunterschied im Mittel von höchstens $r$ Einheiten 
zugunsten der Alkohol-Gruppe führten.

\item Laut dem Lemma können wir nun drei Arten von $p$-Werten berechnen
($p_{\ell}, p_r, p_z$).
Auf den Unterschied zwischen diesen drei Berechnungsarten kommen wir in 
Bemerkung \ref{bem:einzweiseitig} zurück, 
aber im obigen Beispiel haben wir den $p$-Wert berechnet als die Proportion von 
Zuordnungen, 
für die der Gruppenunterschied höchstens $-0.73$ oder mindestens $0.73$ war. 
Da beide Proportionen in diesem Beispiel gleich waren, gilt
\[
  \underbrace{F_0(-0.73)}_{\textrm{Proportion $\leq -0.73$}} + \underbrace{1 - F_0(0.73 -)}_{\textrm{Proportion $\geq 0.73$}} = 2 \cdot F_0(-0.73) = 2 \cdot \min\{F_0(-0.73), 1 - F_0(-0.73 -)\}.
\]
Somit entspricht diese Gesamtproportion dem $p$-Wert $p_z$.
Als Zufallsvariable $P$ in Definition \ref{def:pvalue} verwendeten wir also
\[
  P := 2 \cdot \min\{F_0(X), 1 - F_0(X -)\}.
\]
\end{itemize}

\mypar[Verknüpfung zum Forschungsdesign]{Bemerkung}
Der Hypothesentest, den wir soeben durchgeführt haben,
ist ein \term{Randomisierungstest}. 
Sein Gebrauch wird
durch das Forschungsdesign, genauer gesagt: durch
die uneingeschränkte Randomisierung und die Blindierung,
legitimiert:
\begin{itemize}
\item Wenn es zum Beispiel so gewesen wäre,
dass wir Sandra aus medizinischen Gründen nie der
...
und Nicole unbedingt in der gleichen Kondition hätten getestet werden wollen, 
dann hätte es auch keine 20 möglichen Ergebnisse unter der Nullhypothese gegeben, 
sondern wiederum nur 10:
Alle Permutationen mit Nicole und Michael in anderen Konditionen hätten nicht 
vorkommen können. 
Auch dies hätte man dann berücksichtigen müssen.
...
Dieses Experiment und seine Auswertung illustrieren den Unterschied zwischen
\term{zufälliger Zuordnung} und \term{zufälliger Auswahl}:
Wir haben unsere Versuchspersonen zufällig den Konditionen zugeordnet,
aber wir haben sie nicht zufällig aus irgendeiner Population gewählt.
In vielen Einführungen in die Inferenzstatistik werden $p$-Werte dahingegen
eingeführt, indem man sich vorstellt, dass man zwei Zufallsstichproben aus
zwei Populationen zieht und dass man sich für die Frage interessiert,
ob beide Populationen das gleiche Mittel haben.
Wie jedoch bereits besprochen in Abschnitt \ref{sec:nonrandom} bestehen die
Daten in echten Projekten nur selten aus Zufallsstichproben;
Experimente mit zufälliger Zuordnung sind aber durchaus gängig.

Eine gewisse Vorsicht ist aber geboten.
Wenn wir überzeugendere Evidenz für eine Wirkung der experimentellen
Manipulation gefunden hätten,
dann hätten wir daraus immer noch nicht ohne Weiteres
schliessen können, dass die experimentelle Manipulation einen Effekt in
einer bestimmten \emph{Population} hätte.
Eine solche Schlussfolgerung wäre zumindest verteidigbarer gewesen,
wenn wir sowohl die Versuchspersonen zufällig aus dieser Population 
gewählt hätten (\textit{random sampling})
und diese dann zufällig den Kondition zugewiesen hätten
(\textit{random assignment}). Ohne eine zufällige Auswahl beruht
die Generalisierung auf eine gewisse Population auf einer (oft impliziten) sachlogischen
Argumentation -- nicht auf einer statistischen Gegebenheit. 
Das muss nicht heissen, dass eine solche Schlussfolgerung dann falsch ist. 
Aber es ist wichtig zu erkennen, 
dass es sich hierbei nicht um eine statistische Frage handelt.
...
Wer sich für
die Effizienz didaktischer Methoden interessiert ist,
muss wohl die externe Validität der Untersuchung berücksichtigen.
Aber für etwa experimentelle Psycholog:innen ist externe Validität nicht
...
eines allfälligen Unterschieds zwischen den Gruppen die folgenden:
\begin{itemize}
 ...
 \item Alkohol beeinträchtigt die Artikulationsmotorik.
...
dass die Alternativhypothese auch statistisch vage ist. 
Zum Beispiel besagt sie nicht, 
wie gross eine allfällige, von Alkohol ausgelöste Veränderung der 
Sprechgeschwindigkeit sein könnte. 
Sie besagt nicht einmal, 
ob diese Veränderung eine Beschleunigung oder eine Verlangsamung wäre.
\parend
...
Dank Lemma \ref{lemma:pvalue} erhalten wir einen gültigen $p$-Wert,
indem wir die Wahrscheinlichkeit, dass man das beobachtete Muster
(im Beispiel: einen Gruppenunterschied von 0.73 Silben pro Sekunde) 
\emph{oder ein noch extremeres Muster} feststellen würde, 
\emph{wenn die Nullhypothese tatsächlich stimmen würde}.
Im Beispiel haben wir die alternativen Gruppenunterschiede ja unter der Annahme,
dass der beobachtete Gruppenunterschied nur durch Zufall zu Stande gekommen ist, generiert.
Sämtliche andere Definitionen und Interpretationen des $p$-Wertes sind schlicht 
und einfach falsch.
...
Weitere Missverständnisse besprechen \citet{Goodman2008} und \citet{Greenland2016}.
...
der nicht mit dem alltäglicheren Begriff von praktischer oder theoretischer Signifikanz oder Bedeutung verwechselt werden sollte. 
...
spricht man beim Nullhypothesen testen (engl.: \textit{null hypothesis
...
Wenn nun $H_0$ nicht zutrifft (d.h., das Muster kam
nicht nur durch Zufall zu Stande), dann besteht trotzdem
...
den Regressionsmodellen); das Komplement von $\beta$, also $1-\beta$,
nennt man die statistische \textbf{power} eines Tests.
Die Wahrscheinlichkeit $\beta$ kann nicht ohne Weiteres
festgelegt werden, denn sie hängt von vier Faktoren ab:
...
$p>\alpha$ & OK (\textrm{mind. $1 - \alpha$}) & Fehler der 2.\ Art ($\beta$)\\
\midrule
...
\mypar[Typ-M- und Typ-S-Fehler]{Bemerkung}
...
Um den linksseitigen $p$-Wert zu berechnen, zählt man die Anzahl
Zuordnungen $\ell$, in denen das beobachtete Muster mindestens so klein (oder so negativ)
ist wie in den tatsächlichen Daten. Dann berechnet man
\[
...
Dies garantiert weiter, dass $p_{\ell} \neq 0$.

Um den rechtsseitigen $p$-Wert zu berechnen, zählt man die Anzahl
Zuordnungen $r$, in denen das beobachtete Muster mindestens so gross (oder so positiv)
ist wie in den tatsächlichen Daten. Dann berechnet man
\[
  p_{r} = \frac{r + 1}{m + 1}.
\]
Für den zweiseitigen $p$-Wert berechnet man
\[
  p_z = 2\cdot \min\{p_{\ell}, p_{r}\}.
\]
...
Statt einer \textit{for}-Schleife könnte man auch die \texttt{replicate()}-Funktion
verwenden.
...
l <- sum(mean_diffs_H0 <= mean_difference)
r <- sum(mean_diffs_H0 >= mean_difference)
(p_l <- (l + 1) / (m + 1))
(p_r <- (r + 1) / (m + 1))
(p <- 2 * min(p_l, p_r))
@

\mypar[Randomisierungstest für Unterschied zwischen Varianzen]{Aufgabe}
...
\mypar[Unterschied zwischen mehr als zwei Gruppenmitteln]{Aufgabe}\label{ex:randtest_multiple}
  ...
  (Es macht nicht aus, 
  ob Sie dabei die Stichprobenvarianz oder die Populationsvarianz berechnen. 
  Auch die Summe der Quadrate oder die Standardabweichung könnte man berechnen.)
  Passen Sie nun den Codeabschnitt oben an, um diese Nullhypothese zu testen
  und berechnen Sie einen passenden $p$-Wert.
  
  Hinweis: Verwenden Sie einen rechtsseitigen $p$-Wert. (Wieso?)
  ...
  statt des Unterschieds zwischen diesen berechnen. 
  Wenn ein zweiseitiger Test verwendet wird, 
  ergibt dies das gleiche Resultat, 
  da die Varianz (Standardabweichung) eine streng monoton wachsende 
  Funktion des absoluten Unterschieds ist.
\parend

\mypar[Experiment mit \textit{blocking}]{Aufgabe}
  Wir führen ein Prätest/Posttest-Experiment mit zufälliger Zuordnung durch.
  Statt die $2n$ Versuchspersonen jedoch komplett nach dem Zufallsprinzip 
  ...
  \begin{enumerate}
    ...
    \item Angenommen, die Nullhypothese lautet, dass
          ...
          Erklären Sie in Worten (aber präzise!), 
...
\mypar[Experiment mit Schulklassen]{Aufgabe}
  ...
  es gibt. Beschreiben Sie ausserdem für jede der drei Möglichkeiten einen geeigneten
...
\mypar[Wilcoxon-Rangsummentest]{Beispiel}\label{bm:wilcoxon}
  ...
  durch. Wir berechnen dann die Proportion der Vergleiche, in denen der erste
  Wert höher als der zweite ist; bei einer Bindung (also bei Gleichstand) 
  zählen wir den Vergleich als
  halben Erfolg.
  Die selbst gestrickte Funktion \texttt{cles()} führt diese Berechnung aus; 
  ihr Name verweist nach der Bezeichnung \textit{common-language effect size} \citep{McGraw1992},
  aber diese Statistik wird auch \textit{probability of superiority} genannt:
...
Es stellt sich zum Glück heraus, dass wir eine weniger intuitive aber dafür
...
direkt aus der Rangsumme bestimmen. Dies ist aus komputationeller Sicht nützlich,
denn es geht schneller, die Rangsumme zu berechnen, als die nötigen
$n_1 \cdot n_2$ Vergleiche durchzuführen.
<<>>=
n1 <- length(x1)
n2 <- length(x2)
...
der in R als \texttt{wilcox.test()}
...
wilcox.test(x1, x2)
@
Im Output erscheint eine Statistik $W$ (wie Wilcoxon), die jedoch \emph{nicht}
die Rangsumme darstellt. Stattdessen handelt es sich tatsächlich um die Testgrösse
des äquivalenten \term{Mann--Whitney-Test}, die in der Literatur in der Regel als
$U$ bezeichnet wird. 
Diese steht in einem einfacheren Bezug zum \texttt{cles}-Wert:
<<>>=
cles(x1, x2) * (n1 * n2) # CLES zu U
wilcox.test(x1, x2)$statistic / (n1 * n2) # U zu CLES
@
\parend
...
\mypar[Konfidenzintervall für den \textit{common-language effect size}]{Aufgabe}
  ...
  Hinweis: Berechnen Sie in jedem Bootstrap-Durchgang die Rangsumme 
  oder die $U$-Statistik und konvertieren
...
Um $p$-Werte für andere Muster als Gruppenunterschiede zu berechnen,
kann man sehr ähnliche Verfahren zu Randomisierungstests verwenden.
Diese nennt man \term{Permutationstests}; 
öfters werden beide Begriffe aufgrund ihrer Ähnlichkeit als Synonyme verwendet.
Auf Seite \pageref{ex:poarch} haben Sie
...
Das Vorgehen beim Berechnen von $p$-Werten ist eigentlich
immer gleich und folgt der Schablone, der wir uns dank Lemma \ref{lemma:pvalue}
bedienen dürfen:
Wir wählen eine Testgrösse, generieren die Verteilung dieser Testgrösse
unter der Nullhypothese und berechnen dann $p_{\ell}, p_r$ oder $p_z$.
...
müssen wir zuerst die Verteilung des
Korrelationskoeffizienten generieren, den
wir in dieser Stichprobe feststellen würde,
wenn die zwei Variablen (Flanker und Simon)
unabhängig voneinander wären (= $H_0$). Eine Möglichkeit,
...
library(patchwork) # Grafiken nebeneinander stellen
p1 + p2 + p3
@
...
Also 0.25\% ($p = 0.0025$). Wenn Sie diese Berechnungen
...
sich zudem auch noch anschauen, wie viele der unter
$H_0$ generierten Korrelationskoeffizienten gleich $0.46$ oder kleiner sind.
...
(p <- 2 * min(p_l, p_r))
@
Also 0.5\% ($p = 0.005$). Die Wahrscheinlichkeit,
...
(Wir hätten hier natürlich auch auf die Berechnung von \texttt{p\_l})
verzichten können, da dieser Wert klar grösser als \texttt{p\_r} ist.)
...
\mypar[ABX-Tests]{Beispiel}
  Wir wollen untersuchen, ob ein Lerner des Englischen den Unterschied
  zwischen den Phonemen /\textipa{E}/ (wie in \textit{bet})
  und /\textipa{\ae}/ (wie in \textit{bat}) wahrnimmt.
  Zu diesem Zweck gleisen wir einen ABX-Test auf: Wir erstellen
  eine Liste mit 30 Minimalpaaren, dessen Komponenten sich nur 
  in diesen Phonemen voneinander unterscheiden (\textit{bed}--\textit{bad},
  \textit{fed}--\textit{fad}, \textit{peck}--\textit{pack}, usw.).
  Die insgesamt 60 Wörter werden je zwei Mal von einem Muttersprachler aufgenommen.
  Dem Lerner spielen wir pro Minimalpaar eine Aufnahme des /\textipa{E}/-Wortes
  und eine Aufnahme des /\textipa{\ae}/-Wortes vor, und zwar in einer zufälligen
  Reihenfolge. 
  Anschliessend spielen wir ihm die zweite Aufnahme eines der beiden Wörter vor. 
  Seine Aufgabe besteht darin, zu entscheiden, ob die letzte Aufnahme (`X') zur 
  gleichen Kategorie der ersten (`A') oder der zweiten (`B') Aufnahme gehört.
  Der Lerner kategorisiert 21 der 30 Aufnahmen richtig ($X = 21$).
  Dies könnte zwar darauf hindeuten, dass er gewissermassen den Unterschied
  zwischen /\textipa{E}/ und /\textipa{\ae}/ heraushört, 
  aber wir sollten uns auch mit der Möglichkeit befassen, 
  dass er bloss geraten hat ($= H_0$).
  
  Wenn der Lerner nur geraten hätte, so hätte er bei jedem Minimalpaar eine
  Wahrscheinlichkeit von $p_0 = 0.5$, um richtig zu liegen -- sei es, weil
  er jeweils zufällig `A' oder `B' wählt oder weil er beispielsweise 
  abwechselnd `A' und `B' wählt.
  Aus der Richtigkeit einer Antwort könnte man in diesem Fall nichts herleiten
  bezüglich der Richtigkeit einer anderen Antwort, d.h., die Antworten wären
  unabhängig voneinander. 
  Folglich können wir die Nullhypothese wie folgt ausdrücken:
  \[
    H_0 : X \sim \textrm{Binomial}(30, 0.5).
  \]
  Die übliche Alternativhypothese wäre nun bloss, 
  dass die Erfolgswahrscheinlichkeit nicht gleich $p_0 = 0.5$ ist. 
  In diesem Fall sind wir jedoch nur an der Alternative interessiert,
  dass die Erfolgswahrscheinlichkeit \emph{grösser} als $p_0 = 0.5$ ist.
  
  Die obige Nullhypothese können wir mit einem \term{Binomialtest} überprüfen.
  Unter der Nullhypothese gilt $X \sim \textrm{Binomial}(30, 0.5)$.
  Abbildung \ref{fig:binomialtest} zeigt die $\textrm{Binomial}(30, 0.5)$-Verteilung.
  Wir können nun ablesen bzw.\ recht einfach berechnen, wie wahrscheinlich
  es wäre, mindestens $X = 21$ zu beobachten:
  \[
    \Prob(X \geq 21) = 1 - \Prob(X \leq 20),
  \]
  wobei wir diesen Wert mit \texttt{pbinom()} berechnen können:
<<>>=
1 - pbinom(20, 30, 0.5)
@
  Ein einseitiger Binomialtest liefert also einen rechtsseitigen $p$-Wert von etwa $0.02$.
  
<<echo = FALSE, fig.cap = "Links: Wahrscheinlichkeit der Anzahl Erfolge $k$ bei der Binomial($30$, $0.5$)-Verteilung. Rechts: Verteilungsfunktion derselben Verteilung.\\label{fig:binomialtest}", fig.width = 7, fig.height = 3.3, out.width=".8\\textwidth">>=
k <- 0:30
p0 <- 0.5
X <- 21
par(mfrow = c(1, 2))
...
abline(v = X, lty = "dashed", col = "blue")
plot(k, pbinom(k, max(k), p0), pch = 16, ylim = c(0, 1),
     xlab = "k", ylab = "Prob(X ≤ k)")
segments(k, pbinom(k, max(k), p0), k + 1)
segments(-1, pbinom(X, max(k), p0), X, lty = "dashed", col = "blue")
segments(X, -1, y1 = pbinom(X, max(k), p0), lty = "dashed", col = "blue")
par(op)
@

Mit \texttt{binom.test()} erhalten wir das gleiche Ergebnis:
<<>>=
binom.test(21, 30, 0.5, alternative = "greater")
@
Hantieren wir eine $\alpha$-Schwelle von $0.05$, so würden wir schlussfolgern,
dass der Lerner nicht nur Glück beim Raten hatte und den Unterschied zwischen
den zwei Phonemen gewissermassen wahrnimmt.
\parend

Der Binomialtest aus dem obigen Beispiel ist ein exakter Test, mit dem
man einen Binomialparameter mit einem hypothetischen Referenzwert (im Beispiel
$p_0 = 0.5$) vergleichen kann. 
`Exakt' heisst hier, dass die Anforderung aus Definition \ref{def:pvalue} erfüllt wird.
Im Beispiel wurde ein rechtseitiger Test
verwendet, da wir uns nur für die Alternative interessieren, 
dass die Erfolgswahrscheinlichkeit grösser als $p_0$ ist.
Falls wir uns nur für die Alternative interessieren, dass die Erfolgswahrscheinlichkeit
kleiner als $p_0$ ist, so bietet sich ein linksseitiger Test an.
Für diesen würden wir $\Prob(X \leq x)$ berechnen, mit $x$ der beobachteten Anzahl Erfolge.

Für den zweiseitigen Test können wir mit Lemma \ref{lemma:pvalue} einen gültigen
$p$-Wert als
\[
...
einen anderen gültigen $p$-Wert für den zweiseitigen Test herzuleiten, 
sodass dieser Test mehr \textit{power} hat.
Sei $X$ die Zufallsvariable, welche die Anzahl Erfolge aus $n$ Versuchen darstellt,
und sei $p_0$ die Erfolgswahrscheinlichkeit laut der Nullhypothese.
Nun definieren wir die Zufallsvariable 
\[
  \widetilde{X} := \Prob(Y = X),
\]
wo $Y \sim \textrm{Binomial}(n, p_0)$.
Für die beobachtete Anzahl Erfolge $x$ berechnen wir entsprechend 
den Wert $\widetilde{x} := \Prob(Y = x)$.
Mit Lemma \ref{lemma:pvalue} erhalten wir einen gültigen $p$-Wert,
indem wir $F_0(\widetilde{x})$ berechnen.
Hier ist $F_0$ die Verteilung der Zufallsvariable $\widetilde{X}$ unter
der Nullhypothese.

% Etwas mühsamer ist die Herleitung des zweiseitigen Tests, der sich anbietet,
% wenn wir uns für die Alternative interessieren, dass die Erfolgswahrscheinlichkeit
% nicht gleich $p_0$ ist -- egal, ob grösser oder kleiner.
% Ist die Nullhypothese, dass $X \sim \textrm{Binom}(n, p_0)$, so berechnen wir
% für alle $k = 0, \dots, n$ die Wahrscheinlichkeit
% \[
%   p_k := \Prob(X = k).
% \]
% Für eine beobachtete Anzahl von Erfolgen $x$ berechnen wir den zweiseitigen $p$-Wert
% nun als die Summe aller $p_k$s, für welche $p_k \leq p_x$.
% Auch diese Berechnung lässt sich mit Lemma \ref{lemma:pvalue} rechtfertigen:
% Die Zufallsvariable, die wir betrachten, ist diesmal nicht länger die Anzahl Erfolge,
% sondern die Wahrscheinlichkeit dieser Anzahl Erfolge in einer $\textrm{Binom}(n, p_0)$-Verteilung.
% Die 

\mypar[zweiseitiger Binomialtest]{Beispiel}
  Angenommen, die Nullhypothese lautet 
  \[
  X \sim \textrm{Binomial}(17, 1/3)
  \]
  und wir beobachten drei Erfolge.
  Einen ersten gültigen zweiseitigen $p$-Wert können wir nun wie folgt berechnen:
<<>>=
p_l <- pbinom(3, 17, 1/3)
p_r <- 1 - pbinom(2, 17, 1/3)
(2 * min(p_l, p_r))
@
  Dieser Test hat jedoch weniger statistische \textit{power} als die etwas
  kompliziertere Alternative.
  Für diese berechnen wir zunächst für jedes $k = 0, \dots, 17$ den Wert
  $\Prob(Y = k)$:
<<>>=
prob_k <- dbinom(0:17, 17, 1/3)
@
  Die Verteilungsfunktion $F_0$ der Zufallsvariable $\widetilde{X}$
  bei einem Wert $r$ entspricht der Summe der Wahrscheinlichkeiten
  $\Prob(Y = k)$ mit $\Prob(Y = k) \leq r$.
  Wir interessieren uns für $F_0(\Prob(Y = 3))$ und erhalten somit
<<>>=
sum(prob_k[prob_k <= prob_k[4]])
@
  Einfacher geht es mit \texttt{binom.test()}:
<<>>=
binom.test(3, 17, 1/3)$p.value
@
\parend

Binomialtests kommen zugegebenermassen nicht allzu häufig vor.
Sie illustrieren jedoch schön, wie Signifikanztests durchgeführt werden können,
wenn man die Verteilung des Ausgangs eines Versuchs bzw.\ eines Experiments
unter der Nullhypothese kennt: Man berechnet lediglich, wie ungewöhnlich
das beobachtete Result oder noch extremere Resultate nach dieser Verteilung sind.
In den vorigen Abschnitten haben wir die Verteilungen unter der Nullhypothese
durch stures Rechnen geschätzt; hier konnten wir mit gutem Gewissen eine 
der klassischen Wahrscheinlichkeitsverteilungen einsetzen.

\section{Der exakte Test nach Fisher}
Zum Abschluss dieses Kapitels nehmen wir die Logik des sog.\ exakten Tests
nach Fisher unter die Lupe.

\mypar[Klassifikationsaufgabe]{Beispiel}
  Wieder wollen wir untersuchen, ob ein Lerner des Englischen den Unterschied
  zwischen den Phonemen /\textipa{E}/ (wie in \textit{bet})
  und /\textipa{\ae}/ (wie in \textit{bat}) wahrnimmt.
  Diesmal gleisen wir dazu ein anderes Experiment auf.
  Wir stellen dem Lernenden insgesamt 30 Audiodateien zur Verfügung.
  Zwanzig dieser Audiodateien sind Aufnahmen des Worts \textit{bet},
  die restlichen zehn Audiodateien sind Aufnahmen des Worts \textit{bat}.
  Die Aufgabe des Lernenden ist es, jene zwanzig Audiodateien auszuwählen,
  die Aufnahmen von \textit{bet} sind.
  Der Lernende \emph{muss} genau zwanzig Audiodateien auswählen.
  Die Ergebnisse fassen wir in einer Kreuztabelle zusammen:

\begin{center}
\begin{tabular}{lcc|c}
\toprule
                  & klassifiziert als \textit{bet}  & klassifiziert als \textit{bat}   & Zeilensumme\\
\midrule
ist \textit{bet} & $n_{11} = 15$                    & $n_{12} = 5$                        & $n_{1+} = 20$ \\
ist \textit{bat} & $n_{21} = 5$                     & $n_{22} = 5$                        & $n_{2+} = 10$ \\
\midrule
Spaltensumme      & $n_{+1} = 20$                   & $n_{+2} = 10$                       & $n_{++} = 30$\\
\bottomrule
\end{tabular}
\end{center}

  Eine wichtige Eigenschaft dieses Experiments ist, dass die Zeilen- und Spaltensummen
  im Vorhinein festgelegt wurden: 
  Genau $n_{1+} = 20$ der 30 Audiodateien sind Aufnahmen von \textit{bet} 
  und genau $n_{+1} = 20$ der Audiodateien mussten als \textit{bet} klassifiziert werden.
  Sobald wir einen der Einträge $n_{11}, n_{12}, n_{21}, n_{22}$ bekannt ist,
  können wir die anderen drei Einträge bestimmen.
  Die Zufallsvariable $X$, die wir betrachten, ist der $n_{11}$-Eintrag.
  Unter der Nullhypothese, dass der Lernende aus den 30 Audiodateien
  bloss 20 zufällige Dateien ausgewählt hat, 
  ist diese Zufallsvariable \term{hypergeometrisch} verteilt mit Parametern
  $n_{1+}, n_{2+}$ und $n_{+1}$:\footnote{Je nach Referenzwerk wird diese
  Verteilung anders parametrisiert. Hier verwende ich die Parametrisierung,
  die auch in R verwendet wird.}
\[
  H_0 : X \sim \textrm{Hypergeometrisch}(n_{1+}, n_{2+}, n_{+1}).
\]
  Wenn der Lernende tatsächlich im Stande ist, \textit{bet} von \textit{bat}
  zu unterscheiden, so erwarten wir, dass $n_{11}$ eher grösser als kleiner ist.
  Entsprechend berechnen wir hier, ganz analog wie beim Binomialtest, 
  einen rechtsseitigen $p$-Wert.
  
<<echo = FALSE, fig.cap = "Links: Wahrscheinlichkeit des ersten Tabelleneintrages unter einer Hypergeometrisch($20$, $10$, $20$)-Verteilung. Rechts: Verteilungsfunktion derselben Verteilung.\\label{fig:fishertest}", fig.width = 7, fig.height = 3.3, out.width=".8\\textwidth">>=
k <- 0:20
X <- 15
n1plus <- 20
n2plus <- 10
...
segments(-1, phyper(X, n1plus, n2plus, nplus1), X, lty = "dashed", col = "blue")
segments(X, -1, y1 = phyper(X, n1plus, n2plus, nplus1), lty = "dashed", col = "blue")
par(op)
@

  Abbildung \ref{fig:fishertest} zeigt die Hypergeometrisch($20$, $10$, $20$)-Verteilung.
  Wir können nun ablesen bzw.\ einfach berechnen, wie wahrscheinlich ein
  $n_{11}$-Eintrag von mindestens 15 unter dieser Verteilung wäre:
<<>>=
1 - phyper(15 - 1, 20, 10, 20)
@

Das gleiche Ergebnis erhalten wir, wenn wir die beobachtete Tabelle der
\texttt{fisher.test()}-Funktion übergeben:
<<>>=
fisher.test(
  rbind(c(15, 5),
        c(5, 5)),
...
Übrigens erhält man das gleiche Ergebnis, wenn wir uns auf die 
Kategorisierung der \textit{bat}-Aufnahmen fokussieren würden
und die Parameter der hypergeometrischen Verteilung entsprechend anpassen würden,
denn es gilt auch
\[
  H_0 : n_{22} \sim \textrm{Hypergeometrisch}(n_{2+},n_{1+}, n_{+2}):
\]
...
Für den zweiseitigen Test können wir ähnlich vorgehen wie beim Binomialtest:
Wir definieren die Zufallsvariable
\[
  \widetilde{X} := \Prob(Y = X),
\]
wo $Y \sim \textrm{Hypergeometrisch}(n_{1+}, n_{2+}, n_{+1})$,
und berechnen $\widetilde{x} := \Prob(Y = n_{11})$.
Als $p$-Wert verwendet man dann $F_0(\widetilde{x})$,
wo $F_0$ die Verteilungsfunktion der Zufallsvariable $\widetilde{X}$ unter
der Nullhypothese ist.
Wenn man den \texttt{alternative}-Parameter der \texttt{fisher.test()}-Funktion
unspezifiziert lässt, wird dieser Wert berechnet.

Der exakte Test nach Fisher heisst `exakt', weil die von ihm generierten
$p$-Werte die Bedingung in Definition \ref{def:pvalue} genau erfüllen.
Sein eigentlicher Anwendungsbereich ist recht beschränkt,
da es äusserst selten vorkommt, dass sowohl die Zeilen- als auch die Spaltensumme
der Kreuztabelle schon vor der Datenerhebung festgelegt wurden.
Öfters wird dieser Test jedoch eingesetzt, um Kreuztabellen auszuwerten,
für die nur (zum Beispiel) die Zeilensummen im Vorfeld festgelegt wurden
oder für die weder die Zeilen- noch die Spaltensummen im Vorfeld fixiert wurden.
Der Test ist in solchen Fällen zwar nach wie vor exakt (im Sinne der Definition),
kann aber recht konservativ sein: 
Andere Tests (z.B.\ Boschloos Test) sind je nach Forschungsdesign auch exakt,
aber haben mehr \textit{power}.
Die Literatur zur Auswertung von augenscheinlich einfachen $2 \times 2$-Kreuztabellen ist 
erstaunlich beträchtlich und zum Teil voller Kontroversen;
für eine Übersicht, siehe \citet{Fagerland2017}.