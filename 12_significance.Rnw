...
\mypar[zweiseitiger Binomialtest]{Beispiel}
  ...
  \]
  und wir beobachten 4 Erfolge. Wir berechnen $p_k$ für $k = 0, \dots, 17$:
<<>>=
p_k <- dbinom(0:17, 17, 1/3)
@
  Die Wahrscheinlichkeit $p_4$ ist der 5.\ Eintrag dieses Vektors.
  Der zweiseitige $p$-Wert beträgt folglich
<<>>=
sum(p_k[p_k <= p_k[5]])
@
