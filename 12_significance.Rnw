...
%   können Randomisierungstests bequem durchgeführt werden.
...
%                   distribution = approximate(nresample = m))
% @
...
\mypar[ABX-Tests]{Beispiel}
  ...
  Der Lerner kategorisiert 20 der 30 Aufnahmen richtig ($X = 21$).
  ...
  es wäre, mindestens $X = 20$ zu beobachten:
  \[
...
\mypar[zweiseitiger Binomialtest]{Beispiel}
  Angenommen, die Nullhypothese lautet $X \sim \textrm{Binomial}(17, 1/3)$
