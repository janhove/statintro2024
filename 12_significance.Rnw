...
kann sich ausgewählte Kapitel in \citealp{Oehlert2010}, anschauen)
...
wenn die Nullhypothese stimmen würde. So erhalten wir die Verteilungsfunktion
$F_0$ von $X$. Im obigen Beispiel ermittelten wir $F_0$, indem wir
alle möglichen Zuordnungen durchgeackerten. Der Wert $F_0(r)$ entspräche dann
der Proportion der Zuordnungen, die zu einem Gruppenunterschied im Mittel
von höchstens $r$ Einheiten für die Alkohol-Gruppe führten.
...
Bemerkung \ref{bem:einzweiseitig} zurück, aber im obigen Beispiel 
haben wir den $p$-Wert berechnet als die Proportion von 
Zuordnungen, für die der Gruppenunterschied kleiner als $-0.73$ oder
grösser als $0.73$ war. Da beide Proportionen in diesem Beispiel gleich waren,
gilt
\[
  F_0(-0.73) + (1 - F_0(-0.73 -)) = 2 \cdot F_0(-0.73) = 2 \cdot \min\{F_0(-0.73), 1 - F_0(-0.73 -)\}.
\]
...
spricht man beim Nullhypothesen testn (engl.: \textit{null hypothesis
...
Die Wahrscheinlickeit $\beta$ kann nicht ohne Weiteres
...
$U$ bezeichnet wird. Diese steht in einem einfacheren Bezug zum 
\texttt{cles}-Wert umrechnen:
<<>>=
...
\mypar[ABX-Tests]{Beispiel}
  ...
  durch in diesen Phonemen voneinander unterscheiden (\textit{bed}--\textit{bad},
...
1 - pbinom(20, 30, 0.5)
@
  Ein einseitiger Binomialtest liefert also einen $p$-Wert von etwa $0.02$.
...
$p_0 = 0.5$) vergleichen kann. Im Beispiel wurde ein rechtseitiger Test
