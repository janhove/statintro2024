...
<<echo = FALSE, fig.cap = "Links: Wahrscheinlichkeit der Anzahl Erfolge bei der Binomial($30$, $0.5$)-Verteilung. Rechts: Verteilungsfunktion derselben Verteilung.\\label{fig:binomialtest}", fig.width = 7, fig.height = 3.3, out.width=".8\\textwidth">>=
k <- 0:30
p0 <- 0.5
X <- 21
par(mfrow = c(1, 2))
...
\[
  X \sim \textrm{Hypergeometrisch}(n_{1+}, n_{2+}, n_{+1}).
\]
  ...
  Entsprechend berechnen wir hier, ganz analog wie beim Binomialtest, hier 
...
denn es gilt
\[
  n_{22} \sim \textrm{Hypergeometrisch}(n_{2+},n_{1+}, n_{+2}):
\]
...
\[
  \widetilde{X} := \Prob(Y = X),
\]
wo $Y \sim \sim \textrm{Hypergeometrisch}(n_{1+}, n_{2+}, n_{+1})$,
