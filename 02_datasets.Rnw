...
Öfters muss man die eingelesenen Datensätze anschliessend
mit anderen Datensätzen kombinieren und umgestalten
und in der Regel will man auch noch gewisse Informationen
aus den Datensätzen herauslesen. 
Dieses Kapitel widmet sich diesen Schritten.
...
stehen, ist ein Beispiel eines breiten Datensatzes (10 Spalten
...
Wie Sie sehen, erkennt die \texttt{here()}-Funktion, in welchem Ordner 
sich die .Rproj-Datei befindet. 
Bei Ihnen wird dieser Pfad natürlich anders aussehen.
Die Alternative ohne \texttt{here()} und mit einem relativen Pfad 
...
Oft ist es jedoch besser, die Funktion \texttt{all.equal()} zu verwenden,
um auf Gleichheit zu testen.
\parend

\mypar[Fehlermeldungen]{Bemerkung}
...
Ort gespeichert? Hier ist das Problem, dass der Ordner \texttt{data} und nicht \texttt{Data} heisst. \parend
...
Inspizieren Sie die beiden Datensätze \texttt{participants} und \texttt{items}.

\mypar[unterschiedliche CSV-Formate]{Bemerkung}
Wenn Sie auf einem französisch- oder
...
und welches Symbol als Dezimaltrennzeichen dienen soll. \parend
...
Verwenden Sie die \texttt{View()}-Funktion, 
...
Informationen aus Datensatz \texttt{y} hinzugefügt werden, 
insofern welche vorhanden sind.
Wenn es keine Entsprechung in \texttt{y} gibt, 
erscheint in den hinzugefügten Spalten \texttt{NA}.
Weitere `join'-Funktionen sind die folgenden; 
siehe \url{https://dplyr.tidyverse.org/reference/join.html} für Details:
...
und \texttt{inner\_join()} zum gleichen Resultat führen, aber dies ist nicht immer der Fall.

\mypar[join-Funktionen]{Aufgabe}
Das Ziel dieser Übung ist es, die Unterschiede zwischen
...
rechts <- tibble(B = c(1, 3, 4, 4),
                 C = c(10, NA, 12, 7))
@
    \item Inspizieren Sie die beiden neu kreierten Objekte, z.B.\ mit \texttt{View()} oder indem Sie die Objektnamen auf die Konsole eintragen.

    \item Sagen Sie vorher, wie das Ergebnis der folgenden Befehle aussehen wird.
    Kontrollieren Sie erst \emph{danach} Ihre Antwort, indem Sie die Befehle ausführen.
...
Wenn Sie zwei Datensätze zusammenfügen möchten,
...
Kürzel in RStudio: \textsc{ctrl} + \textsc{shift} + \textsc{m}. \parend
...
Mit den obigen Befehlen werden nur gewisse Zeilen auf der Konsole angezeigt.
...
all_data |>
  filter(Position == 1 | Richtig == 0)
@
Hier ist zu bemerken, dass das logische \textit{oder}, 
das mit \texttt{|} dargestellt wird,
ein inklusives \textit{oder} ist, d.h., 
es bedeutet `$A$ oder $B$ oder auch beide'.
Ein exklusives \textit{oder} (also `$A$ oder $B$, aber nicht beide')
...
\mypar[Daten einlesen]{Aufgabe}
Lesen Sie den Datensatz \texttt{helascot\_skills.csv} als \texttt{skills} ein
und inspizieren Sie seine Struktur.\label{page:skills} \parend
...
\mypar{Aufgabe}
\citet{Slavin2011} berichten die Ergebnisse einer mehrjährigen Evaluationsstudie,
...
in der 1., der 2., der 3. und der 4. Klasse.
Tabellen \ref{tab:slavin_1} bis \vref{tab:slavin_4} 
zeigen einen Teil der Ergebnisse, die \citet{Slavin2011} berichten; 
es handelt sich dabei um die durchschnittlichen Vokabeltestergebnisse der 
getesteten SuS.
...
@
  ...
  als \texttt{vorsichtig} übersetzt hat.   \parend
  \end{enumerate}
...
\begin{itemize}
 ...
 \item Versuchspersonen, die kein einziges französisches Lesetestergebnis
       haben, sollten nicht im resultierenden Datensatz vorkommen;
...
in R loszulegen. \parend

\section{Weiterführende Literatur}
Zum Verwalten von Spread\-sheets, siehe meinen Blogeintrag zu \href{https://janhove.github.io/posts/2015-06-18-preparing-data-for-analysis/}{\textit{Some tips on preparing your data for analysis}} (18.6.2015).
\citet{Broman2017} haben weitere nützliche Hinweise.

Das Referenzwerk schlechthin für die Arbeit mit dem tidyverse
ist Wickham et al.'s \textit{R for Data Science} und
ist gratis verfügbar unter \url{https://r4ds.hadley.nz/}.