...
% \mypar[Gross-Sigmanotation]{Bemerkung}
% Formeln wie diese mögen abschrecken,
% sind in Statistikhandbüchern jedoch gang und gäbe---und gar nicht so schwierig.
% 
% Listen von Beobachtungen einer Variablen (`Vektoren') werden meistens mit
...
Eine zweite praktische Eigenschaft ist die folgende. Sei $X$ und Vektor der
...
Der Median ist zwar linear, aber anhand der Mediane von zwei Vektoren
kann man nicht den Median des zusammengelegten Vektors bestimmen.
Für den Median gibt es unter bestimmten Umständen eine (schwierigere)
Variante des zentralen Grenzwertsatzes.
