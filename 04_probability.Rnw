...
Abbildung \ref{fig:sets} stellt die hier verwendeten mengentheoretischen
Operationen dar:
\begin{align*}
  A \cup B
  &= \textrm{$A$ oder $B$} \\
    ...
    \begin{subfigure}[b]{0.45\textwidth}
     ...
     \fill[blue!20] (-1,0) ellipse (1.8cm and 1.2cm);
      ...
      \node[above] at (3,1.5) {$\Omega$};
        ...
        \label{fig:sub4}
    \end{subfigure}
    \caption{Das Komplement von $A$ (`nicht $A$', $A^c$); 
    das relative Komplement von $B$ in $A$ (`$A$, aber nicht $B$', $A \setminus B$); 
    der Schnitt von $A$ und $B$ (`$A$ und $B$', $A \cap B$); 
    und die Vereinigung von $A$ und $B$ (`$A$ oder $B$', $A \cup B$).}
    \label{fig:sets}
\end{figure}
...
abgeleitet werden; Abbildung \ref{fig:sets} stellt die verwendeten mengentheoretischen
Operationen dar.
\begin{enumerate}
  ...
  festgelegt von den Wahrscheinlichkeiten der Elementarereignisse.
  \parend
\end{enumerate}
...
Die Konstante $C_n$ bestimmen wir wie folgt.
Es muss gelten, dass
\[
...
und den Wahrscheinlichkeiten, die wir diesen zuordnen.
...
Auf die Gründe dafür wollen wir hier nicht eingehen; 
Interessierte können die Suchbegriffe `messbare Menge' und `$\sigma$-Algebra' verwenden.
Praktisch gesehen hält sich das Problem glücklicherweise in Grenzen, 
denn die Teilmengen, die man nicht als Ereignis betrachten kann, 
sind recht schwierig zu konstruieren und für uns nicht relevant. 
In der nächsten Definition verzichten wir daher auf eine nähere Beschreibung der Eigenschaften, welche die Familie von Ereignissen, 
über die man Wahrscheinlichkeitsaussagen machen kann, erfüllen muss.
...
\mypar[Unabhängigkeit (1)]{Definition}\label{def:unab1}
...
denn für jedes Ereignis $B$ gilt $\Omega \cap B = B$. Somit
\[
  \Prob(\Omega \cap B) = \Prob(B) = 1\cdot \Prob(B) = \Prob(\Omega)\Prob(B).\parendeq
\]
...
Begründen Sie Ihre Antwort anhand von Definition \ref{def:unab1}.
\parend

\mypar{Aufgabe}
  Es seien $F$ und $G$ irgendwelche Ereignisse mit $\Prob(F) = 0.6$,
  $\Prob(G) = 0.2$ und $\Prob(F \cup G) = 0.72$. 
  Sind $F$ und $G$ unabhängig?
  Begründen Sie Ihre Antwort anhand von Definition \ref{def:unab1}.
\parend

\mypar[Unabhängigkeit (2)]{Definition}\label{def:unab2} Wir sagen, dass $n$ Ereignisse $A_1, \dots, A_n$
...
\]
In Worten: Für \emph{jede} Auswahl von höchstens $n$ Ereignissen ist die
...
Die nächste Definition brauchen wir nur, 
um später etwa die Voraussetzungen des zentralen Grenzwertsatzes zu verstehen.
...
der es uns erlaubt, ein komplexes Ereignis in handlichere
Ereignisse zu zerlegen.
...
\begin{align*}
  ...
  &= \sum_{k=1}^{\infty} \Prob(A | E_k)\Prob(E_k).\parendeq
\end{align*}
...
dass die erste Karte niedrigeren Ranges als die zweite ist,
...
\mypar[medizinische Tests]{Beispiel}\label{bsp:medtest}
  ...
  der schätzungsweise 0.01\% der Neugeborenen betroffen sind.
...
\begin{align*}
  ...
  &= \frac{\Prob(A|B)\Prob(B)}{\Prob(A|B)\Prob(B) + \Prob(A|B^c)(1 - \Prob(B))}. & [\textrm{totale Wahrscheinlichkeit}]\parendeq
\end{align*}
...
\subsection{Die Verteilungs- und Quantilfunktion}\label{sec:verteilungsfunktion}
...
<<echo = FALSE, fig.cap = "Verteilungsfunktion der Punktzahl einer Jasskarte beim \\textit{Obenabe}.\\label{fig:fx}", fig.width = 0.7*4, fig.height = 0.7*3, out.width=".4\\textwidth">>=
...
\parend
<<echo = FALSE, fig.cap = "Verteilungsfunktion der kontinuierlichen Gleichverteilung auf $[0, 360)$.\\label{fig:contuniform}",  fig.width = 0.8*4, fig.height = 0.8*3, out.width=".4\\textwidth">>=
...
\]
Andere Wahrscheinlichkeitsdichten derselben Verteilung würden das gleiche
Ergebnis liefern.
...
\mypar{Bemerkung}
  ...
  Falls $X$ eine Wahrscheinlichkeitsdichte $f_X$ hat, so gilt
  \[
...
welchen Wert Zufallsvariablen, die einer bestimmten Verteilung folgen,
im Schnitt annehmen. Der Erwartungswert ist die Verallgemeinerung
...
\mypar[Varianz einer Summe $\neq$ Summe der Varianzen]{Beispiel}
  ...
  \]
  Intuitiv ist klar, dass die 
  Zufallsvariablen $G$ und $\overline{G}$ nicht unabhängig sind.
  Diese Intuition wird in der nächsten Definition formalisiert.
\parend
...
\mypar[Unabhängigkeit von Zufallsvariablen]{Definition}
  ...
  jedoch bloss eine Spitzfindigkeit, da es recht schwierig ist, eine Zahlenmenge $E$
...
Sei $n$ eine natürliche Zahl und $\Omega := \{1, \dots, n\}$.
...
\subsection{Die Bernoulliverteilung}\label{sec:bernoulli}
...
Sie hat einen Parameter $p \in [0, 1]$, der die Wahrscheinlichkeit einer $1$
...
\subsection{Die Binomialverteilung}\label{sec:binomial}
...
Da die Ausgänge der ersten $k$ bzw.\ der letzten $n-k$ Bernoulliexperimenten nicht voneinander unterschieden werden
können, gibt es 
\[
...
wir 10 Mal mit einem 6-seitigen Würfel würfeln und dabei zählen, wie oft
...
<<echo = FALSE, fig.cap = "Links: Wahrscheinlichkeit der Anzahl Erfolge bei der Binomial($10$, $1/6$)-Verteilung. Rechts: Verteilungsfunktion derselben Verteilung.\\label{fig:dbinom}", fig.width = 7, fig.height = 3.3, out.width=".8\\textwidth">>=
k <- 0:10
par(mfrow = c(1, 2))
...
\[
  \Var(X) = \frac{(b-a)^2}{12}.
\]
Eine Wahrscheinlichkeitsdichte der Gleichverteilung auf $[a,b]$ ist gegeben
durch
\[
  f_U(x) = 
  \begin{cases}
...
und \texttt{runif()} können unabhängige Beobachtungen aus kontinuierlichen
...
definiert: ihren Erwartungswert $\mu$ und ihre Varianz $\sigma^2$.
...
<<echo = FALSE, fig.cap = "Wahrscheinlichkeitsdichten von vier Normalverteilungen.\\label{fig:dnorm}", fig.width = 6, fig.height = 5, out.width=".6\\textwidth">>=
op <- par(no.readonly = TRUE)
par(mfrow = c(2, 2))
...
<<echo = FALSE, fig.cap = "Die $\\mathcal{N}(3, 16)$-Verteilung. Links: Die Wahrscheinlichkeit, einen Wert unter 0 anzutreffen, entspricht der Fläche unter der Wahrscheinlichkeitsdichte bis 0. Rechts: Mittels der Verteilungsfunktion können wir diesen Wert abrufen.\\label{fig:dnormex}", fig.width = 4*7/4, fig.height = 2.8*7/8, out.width=".8\\textwidth">>=
...
\item Für welchen IQ-Wert gilt, dass 25\% der Bevölkerung einen niedrigeren IQ-Wert und 
                                     75\% einen höheren haben?
...
\mypar{Bemerkung}
  Hat die Zufallsvariable $X$ eine kontinuierliche Verteilung,
  so gilt für das $p$-te Quantil $q := F_X^{-1}(p)$ ($p \in (0,1)$,
  siehe Abschnitt \ref{sec:descriptive_cumprop}), $F_X(q) = p$.
  Hat $X$ keine kontinuierliche Verteilung, so ist es möglich, dass
  $F_X(q) > p$.
\parend
...
\mypar[Würfelwurf]{Beispiel}
  ...
  des durchschnittlichen Wurfs $\sigma^2/7 \approx 0.42$.
...
\mypar{Aufgabe}
  ...
  \begin{enumerate}
    \item Sei $n = 2$. Wie gross ist die tatsächliche Wahrscheinlichkeit, dass
