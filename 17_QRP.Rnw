...
und \citet[][\textit{Science fictions}]{Ritchie2021}.

\section{Sterling et al.~(1995) zu \textit{publication bias}}
...
Befunden abzulehnen.
Dieses \term{publication bias} kann dazu führen,
...
\section{Kerr (1998) zu \textit{hypothesizing after the results are known}}
...
\citeauthor{Kerr1998} als \term{HARKing} -- \textit{hypothesizing
after the results are known}. Als Kosten von HARKing
...
HARKing ist eng verknüpft mit Rosinenpickerei (\textit{cherry picking}).
...
\section{Simmons et al.~(2011) zu intransparenter Flexibilität}
...
\begin{itemize}
 ...
 dann eben nur dasjenige, das für die `besten'
...
Kontrollvariablen oder das Weglassen bzw.\ Behalten einiger Ausreisser),
...
Hypothesen können meistens mehreren statistischen Hypothesen
entsprechen (und vice versa). Diese Unterdeterminiertheit kann dazu
...
\mypar{Aufgabe}
(Präregistrierung und Artikel nebeneinander legen.)
\parend