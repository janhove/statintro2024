...
\textcolor{blue}{siehe Kapitel \ref{ch:logistic}}.}
...
der Quadrate minimiert. Ebenso sind die Mediane jenee Werte,
...
\textcolor{blue}{(Eventuell werden in den Ausblickskapiteln einige dieser weiteren Kriterien
angesprochen.)}
...
\mypar[Mittel minimiert die Summe der Quadrate]{Bemerkung}
  ...
  \[
    ...
    = \sum_{i=1}^n (x_i - \widehat{\beta}_0)^2
    = \sum_{i=1}^n x_i^2 - 2x_i\widehat{\beta}_0 + \widehat{\beta}_0^2.
  \]
  ...
  \begin{align*}
    \frac{\df}{\df \widehat{\beta}_0}  \sum_{i=1}^n \left(x_i^2 - 2x_i\widehat{\beta}_0 + \widehat{\beta}_0^2\right)
    &=  \sum_{i=1}^n \frac{\df}{\df \widehat{\beta}_0}\left(x_i^2 - 2x_i\widehat{\beta}_0 + \widehat{\beta}_0^2\right) \\
    &= \sum_{i=1}^n \left(-2x_i + 2\widehat{\beta}_0\right).
  \end{align*}
  Dieser Ausdruck ist genau dann gleich 0, wenn 
  \[
    \sum_{i=1}^n 2x_i = \sum_{i=1}^n 2\widehat{\beta}_0,
  \]
  also wenn
  \[
    \sum_{i=1}^n x_i = n\widehat{\beta}_0,
  \]
  also wenn
  \[
    \widehat{\beta}_0 = \frac{1}{n}\sum_{i=1}^n x_i.
  \]
...
\widehat{\sigma_{\varepsilon}} = \sqrt{\frac{1}{n - p} \sum_{i = 1}^{n} \widehat{\varepsilon}_i^2},
\end{equation}
wo $p$ die
