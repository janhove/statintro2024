...
siehe Kapitel \ref{ch:uncertainty}) und müssen
...
151 und 182. Wenn wir nun beispielsweise für $\beta_0$ einen beliebigen
...
sum(abs(d$GJT - 135))
@
(Es gilt $y_i = \widehat{\beta_0} + \widehat{\varepsilon_i}$, also $\widehat{\varepsilon_i} = y_i - \widehat{\beta_0}$.)
...
(Eventuell werden in den Ausblickskapiteln einige dieser weiteren Kriterien
angesprochen.)
...
\mypar[Mittel minimiert die Summe der Quadrate]{Bemerkung}
  Dass das Stichprobenmittel die Summe der Quadrate minimiert, sieht man so.
  Die Summe der Quadrate ist eine Summe von quadratischen Funktionen in $\widehat{\beta_0}$
  und daher selber eine quadratische Funktion in $\widehat{\beta_0}$:
  \[
    \sum_{i=1}^n \widehat{\varepsilon_i}^2
    = \sum_{i=1}^n (x_i - \widehat{\beta_0})^2
    = \sum_{i=1}^n x_i^2 - 2x_i\widehat{\beta_0} + \widehat{\beta_0}^2.
  \]
  ...
  \[
    \frac{\df}{\df \widehat{\beta_0}}  \sum_{i=1}^n x_i^2 - 2x_i\widehat{\beta_0} + \widehat{\beta_0}^2
    =  \sum_{i=1}^n \frac{\df}{\df \widehat{\beta_0}}\left(x_i^2 - 2x_i\widehat{\beta_0} + \widehat{\beta_0}^2\right)
    = \sum_{i=1}^n \left(-2x_i + 2\widehat{\beta_0}\right).
  \]
  Dieser Ausdruck ist genau dann gleich 0, wenn 
  \[
    \sum_{i=1}^n 2x_i = \sum_{i=1}^n 2\widehat{\beta_0},
  \]
  also wenn
  \[
    \sum_{i=1}^n x_i = n\widehat{\beta_0}.
  \]
  Also wenn
  \[
    \widehat{\beta_0} = \frac{1}{n}\sum_{i=1}^n x_i = \overline{x}.
  \]
\parend
...
predict(mod.lm)
@
In unserem Fall sind dies lediglich 76 Wiederholungen von $\widehat{\beta_0}$. Der Grund ist dieser:
 \[
 y_i = \widehat{\beta_0} + \widehat{\varepsilon_i}.
 \]
Also 
 \[
 \widehat{\varepsilon_i} = y_i - \widehat{\beta_0}, 
 \]
also 
 \[
 \widehat{y_i} = y_i - \widehat{\varepsilon_i} = y_i - (y_i - \widehat{\beta_0}) = \widehat{\beta_0}.
 \]
...
zu quantifizieren. Da in diesem Fall $\widehat{\beta_0}$ gleich dem
...
for (i in 1:n_bootstrap) {
  ...
  bs_residual <- rnorm(n = 76, mean = 0, sd = sd_residuals)
...
die ausserhalb dem zulässigen Bereich der \texttt{GJT}-Variablen liegen,
